"""Validation and safe, deterministic writing for committed W3 evidence."""
...
W3_RECORD_FORMATS = {"SOAP", "DAP", "BIRP"}
W3_SOAP_SECTIONS = ("subjective", "objective", "assessment", "plan")
W3_RISK_FIELDS = ("current_status", "change_since_last_contact", "follow_up_actions")


class W3AcceptanceError(ValueError):
    """Raised when a report cannot serve as durable W3 evidence."""
...
def _fail(message: str) -> None:
    raise W3AcceptanceError(message)
...
def _is_sensitive_key(key: str) -> bool:
    ...
    if tuple(tokens) in safe_session_fields:
        return False
...
def _validate_w3_fields(fields: Any) -> None:
    _require(isinstance(fields, dict), "structured_result.fields must be an object")
    for field in W3_REQUIRED_FIELDS:
        _require(field in fields, f"structured_result.fields.{field} is required")

    record_format = str(fields.get("record_format") or "").upper()
    _require(record_format in W3_RECORD_FORMATS, "record_format must be SOAP, DAP, or BIRP")

    sections = fields.get("session_sections")
    _require(isinstance(sections, dict), "session_sections must be an object")
    for section in W3_SOAP_SECTIONS:
        _require_nonempty_list(sections.get(section), f"session_sections.{section}")

    risk = fields.get("risk_change")
    _require(isinstance(risk, dict), "risk_change must be an object")
    for field in W3_RISK_FIELDS:
        _require_nonempty_list(risk.get(field), f"risk_change.{field}")

    _require_nonempty_list(fields.get("next_session_focus"), "next_session_focus")
...
def _validate_scenario(scenario: Any) -> None:
    ...
    _require(scenario.get("workflow") == "W3", "scenario.workflow must be W3")
    _require(scenario.get("visible_label") == W3_VISIBLE_LABEL, "scenario.visible_label must be the Chinese W3 counseling-record label")
    ...
    _validate_w3_fields(structured.get("fields"))
...
def validate_web_report(report: dict) -> None:
    """Validate visible local Web acceptance evidence for W3."""
    _validate_base(report, "web")
    _validate_url(report.get("base_url"), hosted=False)


def validate_hosted_report(report: dict) -> None:
    """Validate offline URL form plus full W3 hosted HTTP/model evidence."""
...
def validate_template_report(report: dict, repo_root: Path) -> None:
    """Validate evidence from filling and reopening the repository's real W3 DOCX."""
    _require(isinstance(report, dict), "report must be an object")
    _validate_sanitized(report)
    _require(report.get("report_type") == "template", "report_type must be template")
    _validate_timestamp(report.get("timestamp_utc"))
    _require(report.get("workflow") == "W3", "workflow must be W3")

    source = report.get("source_template")
    _require(isinstance(source, dict), "source_template identity is required")
    relative = source.get("path")
    _require(relative == W3_TEMPLATE_PATH, f"source_template.path must identify {W3_TEMPLATE_PATH}")
    ...
    _require(template.is_file(), "source template must be the real repository W3 DOCX")
    ...
    _require(
        ...
        and set(filled_fields) == set(W3_REQUIRED_FIELDS)
        and len(filled_fields) == len(W3_REQUIRED_FIELDS),
        "filled_fields must cover every canonical W3 field",
    )
    ...
    _require(isinstance(required, dict) and set(required) == set(W3_REQUIRED_FIELDS), "required_content must cover every canonical W3 field")
