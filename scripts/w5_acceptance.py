"""Validation and safe, deterministic writing for committed W5 evidence."""
...
W5_VISIBLE_LABEL = "下次会谈计划"
W5_TEMPLATE_PATH = "docs/w5-next-session-plan-template.docx"
W5_REQUIRED_FIELDS = (
    "selected_framework",
    "session_goal",
    "focus_areas",
...
W5_FRAMEWORKS = {"GENERIC", "CBT", "PSYCHODYNAMIC", "HUMANISTIC", "INTEGRATIVE"}
_SAFE_BUSINESS_KEYS = set(W5_REQUIRED_FIELDS)


class W5AcceptanceError(ValueError):
    """Raised when a report cannot serve as durable W5 evidence."""
...
def _fail(message: str) -> None:
    raise W5AcceptanceError(message)
...
def _validate_w5_fields(fields: Any) -> None:
    ...
    _require(framework in W5_FRAMEWORKS, "selected_framework must be generic, CBT, psychodynamic, humanistic, or integrative")
    for field in W5_REQUIRED_FIELDS:
...
def _validate_scenario(scenario: Any) -> None:
    ...
    _require(scenario.get("workflow") == "W5", "scenario.workflow must be W5")
    _require(scenario.get("visible_label") == W5_VISIBLE_LABEL, "scenario.visible_label must be the Chinese W5 next-session label")
    ...
    _validate_w5_fields(structured.get("fields"))
...
def validate_web_report(report: dict) -> None:
    """Validate visible local Web acceptance evidence for W5."""
    _validate_base(report, "web")
    _validate_url(report.get("base_url"), hosted=False)


def validate_hosted_report(report: dict) -> None:
    """Validate offline URL form plus full W5 hosted HTTP/model evidence."""
...
def validate_template_report(report: dict, repo_root: Path) -> None:
    """Validate evidence from filling and reopening the repository's real W5 DOCX."""
    _require(isinstance(report, dict), "report must be an object")
    _validate_sanitized(report)
    _require(report.get("report_type") == "template", "report_type must be template")
    _validate_timestamp(report.get("timestamp_utc"))
    _require(report.get("workflow") == "W5", "workflow must be W5")

    source = report.get("source_template")
    _require(isinstance(source, dict), "source_template identity is required")
    relative = source.get("path")
    _require(relative == W5_TEMPLATE_PATH, f"source_template.path must identify {W5_TEMPLATE_PATH}")
    ...
    _require(template.is_file(), "source template must be the real repository W5 DOCX")
    ...
    _require(
        ...
        and set(filled_fields) == set(W5_REQUIRED_FIELDS)
        and len(filled_fields) == len(W5_REQUIRED_FIELDS),
        "filled_fields must cover every canonical W5 field",
    )
    ...
    _require(isinstance(required, dict) and set(required) == set(W5_REQUIRED_FIELDS), "required_content must cover every canonical W5 field")
...
def validate_model_eval_report(report: dict, repo_root: Path) -> None:
    """Validate committed W5 real-model evaluation evidence without contacting external services."""
    _require(isinstance(report, dict), "report must be an object")
    _validate_sanitized(report)
    ...
    _require(isinstance(cases, list) and any(str(case).startswith("W5-") for case in cases), "eval_cases must include W5 cases")
