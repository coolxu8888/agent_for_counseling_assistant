...
function Select-Workflow {
    ...
    if (-not $negatedSessionNote) {
        $negatedSessionNote = Test-AnyPattern $Text @(
            ...
            "not asking for (a )?(soap|dap|birp)",
            "rather than (a )?(soap|dap|birp)",
            "instead of (a )?(soap|dap|birp)",
            "don't need (a )?(soap|dap|birp)",
            "do not need (a )?(soap|dap|birp)",
            (U "\u4e0d\u8981\u5199\u6210.*session note"),
    ...
    $negatedInitialInterviewSummary = Test-AnyPattern $Text @(
        ...
        "do not keep.*usual initial interview summary",
        "don't keep.*usual initial interview summary",
        "do not keep.*regular initial interview summary",
        "don't keep.*regular initial interview summary",
        "do not keep.*initial interview summary",
        "don't keep.*initial interview summary",
        "not the fixed initial interview summary template",
        "not the usual initial interview summary",
        "not the regular initial interview summary",
        "not the regular intake summary",
        "rather than (the )?(fixed )?(initial interview summary|intake summary) template",
        "rather than (the )?(usual )?(initial interview summary|intake summary)",
        "rather than (the )?(regular )?(initial interview summary|intake summary)( form| format)?",
        "instead of (the )?(fixed )?(initial interview summary|intake summary) template",
        "instead of (leaving it in )?(the )?(usual )?(initial interview summary|intake summary)( format)?",
        "instead of (leaving it in )?(the )?(regular )?(initial interview summary|intake summary)( form| format)?",
        "do not use.*fixed initial interview summary template",
        "don't use.*fixed initial interview summary template",
        "do not use.*initial interview summary format",
        "don't use.*initial interview summary format",
        "do not use.*regular initial interview summary",
