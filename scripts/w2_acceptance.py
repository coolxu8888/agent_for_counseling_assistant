"""Validation and safe, deterministic writing for committed W2 evidence."""
...
W2_VISIBLE_LABEL = "个案背景（BPS）"
W2_TEMPLATE_PATH = "docs/w2-bps-case-background-template.docx"
W2_REQUIRED_FIELDS = (
    "presenting_concerns",
    "case_overview",
    "bio_psycho_social",
...
W2_BPS_DIMENSIONS = ("biological", "psychological", "social")
W2_DIMENSION_FIELDS = ("known_facts", "working_hypotheses", "information_gaps", "follow_up_questions")
W2_OVERVIEW_FIELDS = ("known_facts", "working_hypotheses", "information_gaps")
W2_RISK_FIELDS = ("observed_clues", "missing_or_unclear", "follow_up_questions")


class W2AcceptanceError(ValueError):
    """Raised when a report cannot serve as durable W2 evidence."""
...
def _fail(message: str) -> None:
    raise W2AcceptanceError(message)
...
def _validate_bps_fields(fields: Any) -> None:
    _require(isinstance(fields, dict), "structured_result.fields must be an object")
    for field in W2_REQUIRED_FIELDS:
        _require(field in fields, f"structured_result.fields.{field} is required")
    _require_nonempty_list(fields.get("presenting_concerns"), "presenting_concerns")
    _require_nonempty_list(fields.get("protective_factors"), "protective_factors")
    _require_nonempty_list(fields.get("recommended_focus"), "recommended_focus")
    _require_nonempty_list(fields.get("boundary_notes"), "boundary_notes")

    overview = fields.get("case_overview")
    _require(isinstance(overview, dict), "case_overview must be an object")
    for key in W2_OVERVIEW_FIELDS:
        _require_nonempty_list(overview.get(key), f"case_overview.{key}")

    bps = fields.get("bio_psycho_social")
    _require(isinstance(bps, dict), "bio_psycho_social must be an object")
    for dimension in W2_BPS_DIMENSIONS:
        value = bps.get(dimension)
        _require(isinstance(value, dict), f"bio_psycho_social.{dimension} is required")
        for key in W2_DIMENSION_FIELDS:
            _require_nonempty_list(value.get(key), f"bio_psycho_social.{dimension}.{key}")

    risk = fields.get("risk_formulation")
    _require(isinstance(risk, dict), "risk_formulation must be an object")
    for key in W2_RISK_FIELDS:
        _require_nonempty_list(risk.get(key), f"risk_formulation.{key}")
...
def _validate_scenario(scenario: Any) -> None:
    ...
    _require(scenario.get("workflow") == "W2", "scenario.workflow must be W2")
    _require(scenario.get("visible_label") == W2_VISIBLE_LABEL, "scenario.visible_label must be the Chinese W2/BPS label")
    ...
    _validate_bps_fields(structured.get("fields"))
...
def validate_web_report(report: dict) -> None:
    """Validate visible local Web acceptance evidence for W2."""
    _validate_base(report, "web")
    _validate_url(report.get("base_url"), hosted=False)


def validate_hosted_report(report: dict) -> None:
    """Validate offline URL form plus full W2 hosted HTTP/model evidence."""
...
def validate_template_report(report: dict, repo_root: Path) -> None:
    """Validate evidence from filling and reopening the repository's real W2 BPS DOCX."""
    _require(isinstance(report, dict), "report must be an object")
    _validate_sanitized(report)
    _require(report.get("report_type") == "template", "report_type must be template")
    _validate_timestamp(report.get("timestamp_utc"))
    _require(report.get("workflow") == "W2", "workflow must be W2")

    source = report.get("source_template")
    _require(isinstance(source, dict), "source_template identity is required")
    relative = source.get("path")
    _require(relative == W2_TEMPLATE_PATH, f"source_template.path must identify {W2_TEMPLATE_PATH}")
    ...
    _require(template.is_file(), "source template must be the real repository W2 DOCX")
    ...
    _require(
        ...
        and set(filled_fields) == set(W2_REQUIRED_FIELDS)
        and len(filled_fields) == len(W2_REQUIRED_FIELDS),
        "filled_fields must cover every canonical W2 field",
    )
    ...
    _require(isinstance(required, dict) and set(required) == set(W2_REQUIRED_FIELDS), "required_content must cover every canonical W2 field")
