"""Run strict acceptance against the repository's real W2 BPS DOCX."""
...
class AcceptanceFailure(RuntimeError):
    """The real template did not satisfy the W2 acceptance contract."""


def inspect_template(template_path: Path, repo_root: Path) -> dict:
    root = Path(repo_root).resolve()
    template = Path(template_path).resolve()
    expected = (root / W2_TEMPLATE_PATH).resolve()
    if template != expected or template.suffix.lower() != ".docx":
        raise AcceptanceFailure(f"template must be the repository's real DOCX: {W2_TEMPLATE_PATH}")
...
def _load_structured(structured_path: Path) -> tuple[dict, dict[str, str]]:
    ...
    if data.get("workflow") != "W2" or data.get("document_type") != "case_summary":
        raise AcceptanceFailure("structured result must be a W2 case_summary")
    ...
    risk = data.get("risk_formulation") or {}
    probes = {
        "presenting_concerns": _first_text(data.get("presenting_concerns"), "presenting_concerns"),
        "case_overview": _first_text(overview.get("known_facts"), "case_overview.known_facts"),
        "bio_psycho_social": _first_text((bps.get("biological") or {}).get("known_facts"), "bio_psycho_social.biological.known_facts"),
        "protective_factors": _first_text(data.get("protective_factors"), "protective_factors"),
        "risk_formulation": _first_text(risk.get("follow_up_questions"), "risk_formulation.follow_up_questions"),
        "recommended_focus": _first_text(data.get("recommended_focus"), "recommended_focus"),
        "boundary_notes": _first_text(data.get("boundary_notes"), "boundary_notes"),
    }
    return data, probes


def _first_text(value, label: str) -> str:
    if not isinstance(value, list) or not value or not isinstance(value[0], str) or not value[0].strip():
        raise AcceptanceFailure(f"{label} requires a nonempty first text probe")
...
def run_template_acceptance(
    ...
    *,
    ...
    allowed_report_dir = (Path(repo_root).resolve() / "eval-results" / "acceptance" / "w2").resolve()
    if report.parent != allowed_report_dir or report.suffix.lower() != ".json":
        raise AcceptanceFailure("report must be a JSON file directly under eval-results/acceptance/w2")
    source = inspect_template(template_path, repo_root)
    _data, probes = _load_structured(structured_path)

    with tempfile.TemporaryDirectory(prefix="w2-template-acceptance-") as tmp:
        tmp_path = Path(tmp)
        ...
        missing = [field for field in W2_REQUIRED_FIELDS if field not in verified]
        if missing:
            raise AcceptanceFailure("filled DOCX is missing canonical W2 content: " + ", ".join(missing))
    ...
    report_data = {
        ...
        "fill": {
            ...
            "filled_fields": list(W2_REQUIRED_FIELDS),
            "helper_filled_fields": _sanitize_helper_items(
                helper_report.get("filled_fields", []),
                ("template_label", "confidence", "location"),
            ),
            "unfilled_fields": _sanitize_helper_items(
