"""Validation and safe, deterministic writing for committed W6 evidence."""
...
W6_VISIBLE_LABEL = "咨询路线图"
W6_TEMPLATE_PATH = "docs/w6-counseling-roadmap-template.docx"
W6_REQUIRED_FIELDS = (
    "selected_framework",
    "overview",
    "phases",
    ...
    "collaboration_or_referral_reminders",
    "missing_information",
    "do_not_do",
    "boundary_notes",
)
W6_FRAMEWORKS = {"GENERIC", "CBT", "PSYCHODYNAMIC", "HUMANISTIC", "INTEGRATIVE"}
_SAFE_BUSINESS_KEYS = set(W6_REQUIRED_FIELDS)


class W6AcceptanceError(ValueError):
    """Raised when a report cannot serve as durable W6 evidence."""


_SENSITIVE_VALUE = re.compile(
    r"(?:\b(?:authorization|cookie|set-cookie)\s*:|\bbearer\s+[A-Za-z0-9._~+/=-]+|\bsk-[A-Za-z0-9_-]+|"
    r"\b(?:(?:[a-z0-9]+[_-])*(?:password|token|cookie|secret|credentials?)"
...
def _fail(message: str) -> None:
    raise W6AcceptanceError(message)
...
def _validate_w6_fields(fields: Any) -> None:
    ...
    _require(framework in W6_FRAMEWORKS, "selected_framework must be generic, CBT, psychodynamic, humanistic, or integrative")
    for field in W6_REQUIRED_FIELDS:
...
def _validate_scenario(scenario: Any) -> None:
    ...
    _require(scenario.get("workflow") == "W6", "scenario.workflow must be W6")
    _require(scenario.get("visible_label") == W6_VISIBLE_LABEL, "scenario.visible_label must be the Chinese W6 roadmap label")
    ...
    _validate_w6_fields(structured.get("fields"))
...
def validate_web_report(report: dict) -> None:
    """Validate visible local Web acceptance evidence for W6."""
    _validate_base(report, "web")
    _validate_url(report.get("base_url"), hosted=False)


def validate_hosted_report(report: dict) -> None:
    """Validate offline URL form plus full W6 hosted HTTP/model evidence."""
...
def validate_template_report(report: dict, repo_root: Path) -> None:
    """Validate evidence from filling and reopening the repository's real W6 DOCX."""
    _require(isinstance(report, dict), "report must be an object")
    _validate_sanitized(report)
    _require(report.get("report_type") == "template", "report_type must be template")
    _validate_timestamp(report.get("timestamp_utc"))
    _require(report.get("workflow") == "W6", "workflow must be W6")

    source = report.get("source_template")
    _require(isinstance(source, dict), "source_template identity is required")
    relative = source.get("path")
    _require(relative == W6_TEMPLATE_PATH, f"source_template.path must identify {W6_TEMPLATE_PATH}")
    ...
    _require(template.is_file(), "source template must be the real repository W6 DOCX")
    ...
    _require(
        ...
        and set(filled_fields) == set(W6_REQUIRED_FIELDS)
        and len(filled_fields) == len(W6_REQUIRED_FIELDS),
        "filled_fields must cover every canonical W6 field",
    )
    ...
    _require(isinstance(required, dict) and set(required) == set(W6_REQUIRED_FIELDS), "required_content must cover every canonical W6 field")
...
def validate_model_eval_report(report: dict, repo_root: Path) -> None:
    """Validate committed W6 real-model evaluation evidence without contacting external services."""
    _require(isinstance(report, dict), "report must be an object")
    _validate_sanitized(report)
    ...
    _require(isinstance(cases, list) and any(str(case).startswith("W6-") for case in cases), "eval_cases must include W6 cases")
