...
class CozeApiServerTest(unittest.TestCase):
    ...
    def test_service_info_exposes_openapi_and_tool_urls(self):
        with patch.object(
            ...
            return_value={"pilot_ready": False, "summary": {"fail_count": 1, "warn_count": 2}, "checks": []},
        ):
        ...
        self.assertEqual(info["deployment_readiness"]["summary"]["fail_count"], 1)
    ...
    def test_handle_web_api_routes_workspace_actions(self):
        handler = FakeHandler()
        ...
        with patch.object(coze_api_server, "require_user", return_value=(user, None)):
            with patch.object(
                coze_api_server,
                "handle_workspace",
                return_value=coze_api_server.json_response({"status": "success", "output_path": "/tmp/workspace.zip"}),
            ) as fake:
        ...
        self.assertEqual(payload["output_path"], "/tmp/workspace.zip")
