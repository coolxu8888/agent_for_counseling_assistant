...
DEMO_SCENARIOS = [
    ...
    },
    {
        ...
        "workflow": "W2",
        "summary": "A supervision boundary where completed intake materials should become a BPS case background instead of staying in the regular initial interview summary, while preserving confidentiality and risk follow-up cues.",
        "input": (
            "Please organize these completed intake materials into a BPS supervision case background with confidentiality limits,"
            " risk follow-up questions, and protective factors; this is for case discussion, not the regular initial interview summary."
        ),
        "output_style": "supervision_summary",
    },
    {
        ...
        "input": (
            "Use completed first interview material to write a supervision case background with protective factors"
            " and risk follow-up questions, not the standard initial interview summary risk block."
        ),
        "output_style": "supervision_summary",
    },
    {
...
EXTRA_NEGATED_RECORD_FORMAT_PATTERNS = [
    ...
    r"not asking for (a )?(soap|dap|birp)",
    r"rather than (a )?(soap|dap|birp)",
    r"instead of (a )?(soap|dap|birp)",
    r"don't need (a )?(soap|dap|birp)",
    r"do not need (a )?(soap|dap|birp)",
    r"\u4e0d\u8981\u5199\u6210.*session note",
    ...
    r"\u5148\u4e0d\u505a.*\u54a8\u8be2\u8bb0\u5f55",
    r"\u5148\u4e0d\u8981\u505a.*session note",
    r"\u5148\u4e0d\u8981\u505a.*progress note",
    r"\u5148\u4e0d\u8981\u505a.*counseling record",
    r"\u5148\u4e0d\u8981\u505a.*\u54a8\u8be2\u8bb0\u5f55",
    r"\u5148\u4e0d\u505a.*soap",
    r"\u5148\u4e0d\u505a.*dap",
    r"\u5148\u4e0d\u505a.*birp",
...
def has_negated_initial_interview_summary_scope(text):
    patterns = [
        ...
        r"do not keep.*usual initial interview summary",
        r"don't keep.*usual initial interview summary",
        r"do not keep.*regular initial interview summary",
        r"don't keep.*regular initial interview summary",
        r"do not keep.*initial interview summary",
        r"don't keep.*initial interview summary",
        r"not the fixed initial interview summary template",
        r"not the usual initial interview summary",
        r"not the regular initial interview summary",
        r"not the regular intake summary",
        r"rather than (the )?(fixed )?(initial interview summary|intake summary) template",
        r"rather than (the )?(usual )?(initial interview summary|intake summary)",
        r"rather than (the )?(regular )?(initial interview summary|intake summary)( form| format)?",
        r"instead of (the )?(fixed )?(initial interview summary|intake summary) template",
        r"instead of (leaving it in )?(the )?(usual )?(initial interview summary|intake summary)( format)?",
        r"instead of (leaving it in )?(the )?(regular )?(initial interview summary|intake summary)( form| format)?",
        r"do not use.*fixed initial interview summary template",
        r"don't use.*fixed initial interview summary template",
        r"do not use.*initial interview summary format",
        r"don't use.*initial interview summary format",
        r"do not use.*regular initial interview summary",
        r"don't use.*regular initial interview summary",
        r"do not leave it in.*regular initial interview summary",
        r"don't leave it in.*regular initial interview summary",
        r"not the standard initial interview summary risk block",
        r"not the standard intake summary risk block",
        r"do not leave it in.*standard initial interview summary risk block",
