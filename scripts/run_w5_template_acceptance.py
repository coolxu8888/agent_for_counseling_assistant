"""Run strict acceptance against the repository's real W5 next-session-plan DOCX."""
...
from w5_acceptance import W5_REQUIRED_FIELDS, W5_TEMPLATE_PATH, validate_template_report, write_sanitized_report
...
class AcceptanceFailure(RuntimeError):
    """The real template did not satisfy the W5 acceptance contract."""


def inspect_template(template_path: Path, repo_root: Path) -> dict:
    root = Path(repo_root).resolve()
    template = Path(template_path).resolve()
    expected = (root / W5_TEMPLATE_PATH).resolve()
    if template != expected or template.suffix.lower() != ".docx":
        raise AcceptanceFailure(f"template must be the repository's real DOCX: {W5_TEMPLATE_PATH}")
    ...
    return {"path": W5_TEMPLATE_PATH, "sha256": hashlib.sha256(template.read_bytes()).hexdigest()}
...
def _load_structured(structured_path: Path) -> tuple[dict, dict[str, str]]:
    ...
    if data.get("workflow") != "W5" or data.get("document_type") != "next_session_plan":
        raise AcceptanceFailure("structured result must be a W5 next_session_plan")
    if data.get("deidentified") is not True:
        raise AcceptanceFailure("acceptance fixture must be explicitly marked deidentified")
    probes = {field: _first_text(data.get(field), field) for field in W5_REQUIRED_FIELDS}
    return data, probes
...
def run_template_acceptance(
    ...
    *,
    ...
    allowed_report_dir = (Path(repo_root).resolve() / "eval-results" / "acceptance" / "w5").resolve()
    if report.parent != allowed_report_dir or report.suffix.lower() != ".json":
        raise AcceptanceFailure("report must be a JSON file directly under eval-results/acceptance/w5")
    source = inspect_template(template_path, repo_root)
    _data, probes = _load_structured(structured_path)

    with tempfile.TemporaryDirectory(prefix="w5-template-acceptance-") as tmp:
        tmp_path = Path(tmp)
        ...
        missing = [field for field in W5_REQUIRED_FIELDS if field not in verified]
        if missing:
            raise AcceptanceFailure("filled DOCX is missing canonical W5 content: " + ", ".join(missing))
    ...
    report_data = {
        ...
        "fill": {
            ...
            "filled_fields": list(W5_REQUIRED_FIELDS),
