"""Run strict acceptance against the repository's real W3 SOAP/DAP/BIRP DOCX."""
...
class AcceptanceFailure(RuntimeError):
    """The real template did not satisfy the W3 acceptance contract."""


def inspect_template(template_path: Path, repo_root: Path) -> dict:
    root = Path(repo_root).resolve()
    template = Path(template_path).resolve()
    expected = (root / W3_TEMPLATE_PATH).resolve()
    if template != expected or template.suffix.lower() != ".docx":
        raise AcceptanceFailure(f"template must be the repository's real DOCX: {W3_TEMPLATE_PATH}")
...
def _section_content(sections: list, section_id: str) -> str:
    for section in sections:
        if isinstance(section, dict) and str(section.get("id") or "").lower() == section_id:
            return _first_text(section.get("content"), f"sections.{section_id}.content")
    raise AcceptanceFailure(f"sections.{section_id} is required")
...
def _load_structured(structured_path: Path) -> tuple[dict, dict[str, str]]:
    ...
    if data.get("workflow") != "W3" or data.get("document_type") != "session_note":
        raise AcceptanceFailure("structured result must be a W3 session_note")
    if data.get("deidentified") is not True:
        raise AcceptanceFailure("acceptance fixture must be explicitly marked deidentified")
    record_format = str(data.get("record_format") or "").upper()
    if record_format not in {"SOAP", "DAP", "BIRP"}:
        raise AcceptanceFailure("record_format must be SOAP, DAP, or BIRP")
    sections = data.get("sections")
    if not isinstance(sections, list):
        raise AcceptanceFailure("sections must be a list")
    risk = data.get("risk_change") or {}
    probes = {
        "record_format": record_format,
        "session_sections": _section_content(sections, "subjective"),
        "risk_change": _first_text(risk.get("change_documentation"), "risk_change.change_documentation"),
        "next_session_focus": _first_text(data.get("next_session_focus"), "next_session_focus"),
...
def run_template_acceptance(
    ...
    *,
    ...
    allowed_report_dir = (Path(repo_root).resolve() / "eval-results" / "acceptance" / "w3").resolve()
    if report.parent != allowed_report_dir or report.suffix.lower() != ".json":
        raise AcceptanceFailure("report must be a JSON file directly under eval-results/acceptance/w3")
    source = inspect_template(template_path, repo_root)
    _data, probes = _load_structured(structured_path)

    with tempfile.TemporaryDirectory(prefix="w3-template-acceptance-") as tmp:
        tmp_path = Path(tmp)
        ...
        missing = [field for field in W3_REQUIRED_FIELDS if field not in verified]
        if missing:
            raise AcceptanceFailure("filled DOCX is missing canonical W3 content: " + ", ".join(missing))
    ...
    report_data = {
        ...
        "fill": {
            ...
            "filled_fields": list(W3_REQUIRED_FIELDS),
