...
`workflow-completion.json` 是 W1-W6 门槛状态的唯一权威来源。每个工作流只有在本地测试、真实模型评测、Web 集成、托管验证和真实模板验证五个门槛全部通过后才算完成；完成状态由程序推导，不手工填写。证据变化后，先运行 `python scripts/workflow_completion.py --write` 重新生成下表，再运行 `python scripts/workflow_completion.py --check` 检查数据与文档是否一致。
...
| P0 | W1 initial interview preparation guide | 历史交付：`shipped partial`；当前完成状态[以统一矩阵为准](#w1-w6-unified-completion-matrix) | W1 now extracts partial intake clues, prefills the intake guide contract, exposes an explicit product-facing prep-mode summary, and passes live DeepSeek eval `W1-007` plus a real structured run | extend bilingual clue extraction coverage and verify the hosted deployment shows the new prep-mode summary |
| P0 | W1 initial interview summary into fixed template | 历史交付：`shipped partial`；当前完成状态[以统一矩阵为准](#w1-w6-unified-completion-matrix) | W1 now normalizes collapsed summary sections back into the fixed template, auto-fills missing split fields, exposes a dedicated `W1 summary brief` in the workbench, and passes live DeepSeek evals `W1-005` and `W1-009` plus a real structured run with `structured_status=PASS` | verify the hosted deployment uses the new summary brief and broaden section-label normalization for more bilingual raw-note variants |
| P0 | W2 case background organization with BPS | 历史交付：`shipped partial`；当前完成状态[以统一矩阵为准](#w1-w6-unified-completion-matrix) | dedicated BPS structure, AUTO routing, DOCX rendering, split-template alias coverage, and live evals `W2-005` plus `W2-006` now ship in runner/web/eval | verify hosted deployment and extend more real counselor template label coverage |
| P0 | W3 session summary and counseling record | 历史交付：`shipped partial`；当前完成状态[以统一矩阵为准](#w1-w6-unified-completion-matrix) | generic + SOAP + DAP structured paths now also include product-facing BIRP record summaries, a dedicated BIRP demo, cleaner/eval coverage through `W3-007`, and existing risk-change documentation plus DOCX/template mapping | run a live DeepSeek `W3-007` eval when credentials are available and verify the hosted deployment |
| P0 | W4 case conceptualization by theory/framework | 历史交付：`shipped partial`；当前完成状态[以统一矩阵为准](#w1-w6-unified-completion-matrix) | `W4` shipped in runner/web/RAG/eval and now includes humanistic + psychodynamic retrieval-backed boundary coverage (`W4-002`, `W4-003`) plus dedicated W5/W6 sister framework cards to keep conceptualization separate from planning retrieval | add more per-framework subtopic cards and hosted verification |
| P0 | W5 bounded next-session plan | 历史交付：`shipped partial`；当前完成状态[以统一矩阵为准](#w1-w6-unified-completion-matrix) | `W5` shipped in runner/web/RAG/eval and now includes psychodynamic + integrative theory-specific planning retrieval coverage (`W5-003`, `W5-004`) with dedicated framework planning chunks | verify hosted deployment and extend more bilingual framework-routing coverage |
| P0 | Counseling roadmap / multi-session plan | 历史交付：`shipped partial`；当前完成状态[以统一矩阵为准](#w1-w6-unified-completion-matrix) | `W6` shipped in runner/web/RAG/eval and now includes humanistic + psychodynamic roadmap retrieval coverage (`W6-003`, `W6-004`) with dedicated framework roadmap chunks | verify hosted deployment and extend more framework-specific roadmap source cards |
...
- `Intent recognition across counselor tasks`, specifically the remaining hosted-parity gap for the `W2-012` and `W2-013` completed-intake-material boundaries where public Render still needed to prove `W2` routing with visible mixed-signal metadata.

What changed:

- Tightened the hosted verification contract in [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\hosted_smoke.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\hosted_smoke.py) so public smoke checks can now assert:
  - `route_status`
  - `route_notice`
  - the existing `detected_workflow` and `routing_reasons_summary` fields together
- Mirrored the new hosted assertions through [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\run-hosted-smoke.ps1`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\run-hosted-smoke.ps1) so the PowerShell wrapper can validate the same contract during future deployment checks.
- Rebuilt [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_hosted_smoke.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_hosted_smoke.py) with clean UTF-8-safe fixtures and new assertions covering `expect_route_status` and `expect_route_notice_substring`.
- Pushed the previously local `W2-012` / `W2-013` routing commits plus the new hosted-smoke contract to `origin/main`, then re-polled the public Render deployment until both hosted AUTO boundaries returned the expected `W2` route metadata.

Tests and evals run:

- `$env:PYTHONPATH='scripts'; python -m unittest scripts.test_hosted_smoke.HostedSmokeTest.test_parse_args_accepts_route_metadata_expectations scripts.test_hosted_smoke.HostedSmokeTest.test_run_smoke_requires_expected_auto_route_metadata`
- `$env:PYTHONPATH='scripts'; python -m unittest scripts.test_hosted_smoke` -> 7 tests passed.
- `$env:PYTHONPATH='scripts'; python -m unittest scripts.test_web_workbench.WebWorkbenchTest.test_detect_workflow_prefers_w2_for_chinese_loose_initial_interview_summary_negation scripts.test_web_workbench.WebWorkbenchTest.test_detect_workflow_marks_mixed_signals_for_regular_initial_interview_summary_negation scripts.test_run_retrieval.RunRetrievalTest.test_routes_chinese_loose_initial_interview_summary_negation_to_w2 scripts.test_run_retrieval.RunRetrievalTest.test_routes_regular_initial_interview_summary_negation_case_background_to_w2 scripts.test_build_workflow_eval_prompts.BuildWorkflowEvalPromptsTest.test_evals_include_w2_chinese_loose_initial_interview_summary_negation_boundary_case scripts.test_build_workflow_eval_prompts.BuildWorkflowEvalPromptsTest.test_evals_include_w2_regular_initial_interview_summary_negation_boundary_case scripts.test_clean_eval_outputs.CleanEvalOutputsTest.test_w2_012_chinese_loose_initial_interview_summary_negation_background_case_passes_rules_and_rubric scripts.test_clean_eval_outputs.CleanEvalOutputsTest.test_w2_013_regular_initial_interview_summary_negation_background_case_passes_rules_and_rubric` -> 8 tests passed.
- `$env:PYTHONPATH='scripts'; python scripts/hosted_smoke.py --base-url https://counselor-agent-coze-api.onrender.com --username demo --password demo123 --workflow AUTO --input "把这份已完成的首访材料整理成督导讨论用的BPS个案背景，保留已知事实、信息缺口、保护因素和风险追问，不要还是按常规初访总结。" --expect-detected-workflow W2 --expect-route-status mixed_signals --expect-route-notice-substring "Case background" --expect-route-summary-substring "W2 Case background" --real-run --timeout 240` -> passed after redeploy.
- `$env:PYTHONPATH='scripts'; python scripts/hosted_smoke.py --base-url https://counselor-agent-coze-api.onrender.com --username demo --password demo123 --workflow AUTO --input "Please organize these completed intake materials into a BPS supervision case background with confidentiality limits, risk follow-up questions, and protective factors; this is for case discussion, not the regular initial interview summary." --expect-detected-workflow W2 --expect-route-status mixed_signals --expect-route-notice-substring "Case background" --expect-route-summary-substring "W2 Case background" --real-run --timeout 240` -> passed after redeploy.

Outcome:

- Hosted parity is now restored for both remaining `W2` intent-routing boundaries. The public Render product returns `workflow=W2`, `detected_workflow=W2`, `route_status=mixed_signals`, a `Case background` `route_notice`, and the expected `W2 > W1` `routing_reasons_summary` for both `W2-012` and `W2-013`.
- The deployment-smoke tooling now checks the same route metadata that counselors rely on in the product, instead of only verifying the top workflow id.
...
- `Intent recognition across counselor tasks`, specifically the W1-vs-W2 mixed-risk boundary where completed first-interview material was explicitly requested as a supervision BPS background `not the standard initial interview summary risk block`.

What changed:

- Added a dedicated product-facing demo entry for this boundary in [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\web_workbench.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\web_workbench.py) and [`C:\Users\win\Documents\Codex\2026-05-15\agent\web-workbench\app.js`](C:\Users\win\Documents\Codex\2026-05-15\agent\web-workbench\app.js) so the shipped workbench now exposes `completed first interview material -> supervision BPS background (standard risk-block negation)` directly in the demo catalog.
- Tightened the product-side AUTO router in [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\web_workbench.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\web_workbench.py) by broadening `has_negated_initial_interview_summary_scope()` to recognize English and Chinese risk-block negation phrasing such as `not the standard initial interview summary risk block`, `do not leave it in the standard initial interview summary risk block`, and `不要沿用初访总结里的风险栏`, so `W2` now preserves visible `W2 > W1` mixed-signal routing metadata instead of flattening to a `clear` route.
- Brought retrieval parity into [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\run-retrieval.ps1`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\run-retrieval.ps1) so the same mixed-risk supervision prompt now routes to `workflow_2_case_summary` instead of falling back to `workflow_1_intake_form`.
- Expanded eval coverage with `W2-014` in [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\build_workflow_eval_prompts.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\build_workflow_eval_prompts.py), regenerated committed assets including [`C:\Users\win\Documents\Codex\2026-05-15\agent\eval-prompts\W2-014-standard-initial-interview-summary-risk-block-negation-case-background.txt`](C:\Users\win\Documents\Codex\2026-05-15\agent\eval-prompts\W2-014-standard-initial-interview-summary-risk-block-negation-case-background.txt) and the updated [`C:\Users\win\Documents\Codex\2026-05-15\agent\eval-prompts\manifest.json`](C:\Users\win\Documents\Codex\2026-05-15\agent\eval-prompts\manifest.json).
- Added scorer/rubric coverage for `W2-014` in [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\clean_eval_outputs.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\clean_eval_outputs.py) plus regression coverage in:
  - [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_web_workbench.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_web_workbench.py)
  - [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_run_retrieval.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_run_retrieval.py)
  - [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_build_workflow_eval_prompts.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_build_workflow_eval_prompts.py)
  - [`C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_clean_eval_outputs.py`](C:\Users\win\Documents\Codex\2026-05-15\agent\scripts\test_clean_eval_outputs.py)
...
- `$env:PYTHONPATH='scripts'; python -m unittest scripts.test_web_workbench.WebWorkbenchTest.test_detect_workflow_marks_mixed_signals_for_standard_initial_interview_summary_risk_block_negation scripts.test_run_retrieval.RunRetrievalTest.test_routes_standard_initial_interview_summary_risk_block_negation_case_background_to_w2 scripts.test_build_workflow_eval_prompts.BuildWorkflowEvalPromptsTest.test_evals_include_w2_standard_initial_interview_summary_risk_block_boundary_case scripts.test_clean_eval_outputs.CleanEvalOutputsTest.test_w2_014_standard_initial_interview_summary_risk_block_background_case_passes_rules_and_rubric` -> failed red first, then passed green.
- `$env:PYTHONPATH='scripts'; python -m unittest scripts.test_web_workbench scripts.test_run_retrieval scripts.test_build_workflow_eval_prompts scripts.test_clean_eval_outputs` -> 210 tests passed.
- `node --check web-workbench/app.js`
- `$env:PYTHONPATH='scripts'; python scripts/build_workflow_eval_prompts.py`
- Loaded `.env` into the process environment without printing secret values, then ran:
  - `$env:PYTHONPATH='scripts'; $env:DEEPSEEK_TIMEOUT_SECONDS='240'; python scripts/run_model_eval.py --ids W2-014` -> passed.
  - `$env:PYTHONPATH='scripts'; python scripts/hosted_smoke.py --base-url https://counselor-agent-coze-api.onrender.com --username demo --password demo123 --workflow AUTO --input "Use completed first interview material to write a supervision case background with protective factors and risk follow-up questions, not the standard initial interview summary risk block." --expect-detected-workflow W2 --expect-route-status mixed_signals --expect-route-notice-substring "Case background" --expect-route-summary-substring "W2 Case background" --real-run --timeout 240` -> passed after one transient Render `502` during redeploy.
